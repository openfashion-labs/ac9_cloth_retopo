...
def invalidate_guide_cache(guide_obj=None) -> None:
    ...
    Also drops the memoised flat/real ratio, which every caller that converts a
    real-space tolerance into flat-layout units depends on: re-creating the
    Flat SK can change the ratio without changing the vertex counts the memo
    keys on, so it has to be cleared wherever the Guide's flat layout is
    declared stale — and this is that one place.
    """
    _guide.invalidate_flat_per_real(guide_obj)
...
def _get_guide_cache(guide_obj, flat_sk: str) -> dict:
    ...
    if key not in _guide_cache:
        ...
        if bvh_2d.n_excluded:
            # Guide quality, reported once per cache build: CLO exports carry
            # zero-area slivers and a retopo vert landing on one used to be
            # flung to its 2D layout position. They are excluded from the BVH
            # now, but the count is worth seeing when a Guide is swapped.
            print(
                f"[AC9 CLO Projector] Guide cache '{guide_obj.name}': "
                f"{len(tris_2d)} triangles, {bvh_2d.n_excluded} degenerate in "
                f"2D excluded from the BVH"
            )
...
class ProjectionResult:
    ...
    failed_indices: Tuple = ()        # vertices with no usable attachment
...
def run_forward_projection(
    ...
    *,
    ...
    # Progress once per percent, not per vertex: the callback redraws the GUI
    # cursor (~1.4 ms each), see compute_attachments in attachment.py.
    last_pct = -1
    ...
    failed = tuple(i for i, a in enumerate(attachments) if not a.is_ok)
    return ProjectionResult(
        success=True,
        total=n,
        projected=n - len(failed),
        failed=len(failed),
...
def run_reverse_projection(
    ...
    `progress`, if given, is called with a 0..1 fraction across the re-bind —
    the long pole, and the reason this operator sat silent for 7 s on a
    172 k-triangle Guide. Pass a ui_common.ProgressThrottle.
    """
    if retopo is None or retopo.type != "MESH":
        return ProjectionResult(success=False, error="2D Retopo Object must be a mesh.")
...
def _update_failed_group(retopo, retopo_mesh, attachments, clear_failed_group, select_failed):
    if FAILED_GROUP_NAME in retopo.vertex_groups:
        ...
        if clear_failed_group:
            # Empty it in place.  Deleting + re-creating the group changes the
            # object's vertex-group LIST, which tags a depsgraph relations
            # rebuild — every object in the scene gets re-evaluated and its GPU
            # batches rebuilt, on every Refresh.  vg.remove() only touches this
            # object's geometry.
            vg.remove(list(range(len(retopo_mesh.vertices))))
    else:
...
def ngon_stats(mesh):
    """Return (count, largest) over the faces with more than four verts.

    Blender's Edit-Mode Subdivide splits an n-gon's OUTLINE but never its
    interior: a 146-gon comes back as a 292-gon of the same area with not one
    new edge inside it (measured on the production file, 2026-09-07). The
    region then reads as "the wireframe vanished here" next to quads that did
    quadruple, which looks exactly like a bug.

    Subdividing them automatically is not the answer — a centre-point fan
    would put 146 edges on one pole, and the honest cut is the user's own
    (Knife -> Connect). So Subdivide keeps passing them through untouched,
    on purpose: an unfinished panel must never stop the user from checking
    the silhouette and the poly count of everything else. Counting them here
    is what turns a silent surprise into a result line.
    """
    sizes = [len(p.vertices) for p in mesh.polygons if len(p.vertices) > 4]
    return len(sizes), (max(sizes) if sizes else 0)


def ngon_note(mesh):
    """One clause for a result line, or "" when the mesh is all tris/quads."""
    count, largest = ngon_stats(mesh)
    if not count:
        return ""
    return (f"  {count} n-gon(s) left whole (largest {largest}v) — Subdivide "
            f"only cuts their outline; cut them to grid them.")
