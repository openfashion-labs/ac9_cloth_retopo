...
Depends on shapely (already required by flat_merge) for point-in-polygon and
...
def clear_fill(bm):
    ...
    if flay is not None:
        dead = [f for f in bm.faces if f[flay]]
        n_f = len(dead)
        if dead:
            bmesh.ops.delete(bm, geom=dead, context='FACES_ONLY')
    if vlay is not None:
        bm.verts.ensure_lookup_table()
        dead = [v for v in bm.verts if v[vlay]]
        n_v = len(dead)
        if dead:
...
def fill_regions(bm, retopo, island_of, target_mm, margin_factor=0.6,
                 quad_angle=math.pi, epsilon=1e-6):
    """Fill every panel region with a quad-dominant mesh. Returns a report.

    target_mm is the wanted interior edge length; pass the same spacing the
    boundary was generated at and the interior matches it.
    """
    ...
    for isl, edges in by_island.items():
        pos = {}
        for a, b in edges:
            pos[a] = vref[a].co
            pos[b] = vref[b].co
...
def run_preview_fill(guide, retopo, flat_sk, target_mm, margin_factor=0.6,
    ...
    """Clear any previous fill and lay a fresh one. Returns the report."""
    me = retopo.data
    bm = bmesh.new()
    bm.from_mesh(me)
    bm.verts.ensure_lookup_table()

    cleared_f, cleared_v = clear_fill(bm)
    bm.verts.ensure_lookup_table()
