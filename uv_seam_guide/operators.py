...
class AC9_OT_refresh_ghosts(Operator):
    ...
    def execute(self, context):
        ...
        count, total = rebuild_ghosts(props, retopo_obj)
        _tag_redraw_3d(context)
        self.report(
            {'INFO'},
            f"Ghost overlay: {count} ghosts from {total} boundary retopo vertices.",
        )
        return {'FINISHED'}
...
class AC9_OT_clear_uv_seam_guides(Operator):
    ...
    def execute(self, context):
        ...
        _batches["cross_unplaced"] = _batches["cross_placed"] = None
        _batches["conn"] = None
...
class AC9_OT_ghost_force_bond(Operator):
    ...
    bl_description = (
        ...
        "roughly but forgot to snap exactly"
    )
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        for v in bm.verts:
            ...
            for ghost in ghosts:
                ...
                if d < best_d:
                    best_d = d
                    best_g = g
            if best_g is not None and best_d <= bond:
        ...
        _tag_redraw_3d(context)
        self.report(
            {'INFO'} if moved else {'WARNING'},
            f"Force-bonded {moved} vertex(es); {skipped} selected vert(s) had "
            f"no ghost within Snap Distance ({bond * 1000:.1f} mm).",
        )
        return {'FINISHED'}
...
class AC9_OT_analyze_guide(Operator):
    ...
    def execute(self, context):
        ok_sym = 'FINISHED' in bpy.ops.ac9_cloth.detect_island_symmetry('EXEC_DEFAULT')
        ok_mirror = 'FINISHED' in bpy.ops.ac9_cloth.detect_mirror_pairs('EXEC_DEFAULT')
...
class AC9_OT_analyze_all(Operator):
    ...
    def execute(self, context):
        ...
        try:
            ...
            ok_seams = 'FINISHED' in bpy.ops.ac9_cloth.analyze_uv_seam_pairs('EXEC_DEFAULT')
            wm.progress_update(35)
            ok_sym = 'FINISHED' in bpy.ops.ac9_cloth.analyze_guide('EXEC_DEFAULT')
            wm.progress_update(85)
            ok_anchors = 'FINISHED' in bpy.ops.ac9_cloth.analyze_anchors('EXEC_DEFAULT')
...
class AC9_OT_mirror_pair_retopo_map(Operator):
    ...
    def execute(self, context):
        ...
        text = bpy.data.texts.get(self.TEXT_NAME) or bpy.data.texts.new(self.TEXT_NAME)
        text.clear()
        text.write("\n".join(lines))
...
class AC9_OT_ghost_snap_move(Operator):
    ...
    bl_description = (
        ...
        "Snap Distance. Activated by G when Ghost Snap Mode is ON"
    )
    bl_options = {'REGISTER', 'UNDO'}
...
class AC9_OT_SyncRetopoSeams(bpy.types.Operator):
    """Give the retopo's two sides of each sewn seam matching vertices.

    Uses the Guide's anchor spans as the specification: wherever one side of
    a seam has retopo vertices the other side lacks, they are created on the
    partner side at the same position along the seam, so the two sides
    correspond one-to-one and coincide in 3D.

    Where a seam is authored on one side only, the whole partner chain is
    laid down. Nothing is moved or deleted — existing retopo vertices stay
    exactly where they are
    """

    bl_idname = "ac9_cloth.sync_retopo_seams"
    bl_label = "Sync Seams"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        ...
        try:
            ...
            res = retopo_seam_sync.run_retopo_seam_sync(
                guide, retopo, flat_sk,
                tol=props.retopo_sync_tol,
                dry_run=not self.apply,
            )
            wm.progress_update(100)
        except Exception as exc:
            self.report({"ERROR"}, f"Retopo seam sync failed: {exc}")
            return {"CANCELLED"}
        finally:
            wm.progress_end()
        ...
        skipped = res["skipped_interleaved"]
        skip_note = ""
        if skipped:
            skip_note = (
                f" {skipped} seam(s) left alone: both sides are authored but "
                f"their vertices interleave, so mirroring would roughly double "
                f"the count instead of fixing it — those need repositioning. "
                f"Run Seam Status to list them."
            )

        if n == 0:
            top.status_boundary = (
                f"Nothing to add ({skipped} need repositioning)"
                if skipped else "Every sewn seam already matches."
            )
            self.report(
                {"WARNING"} if skipped else {"INFO"},
                ("Nothing to mirror." + skip_note) if skipped
                else "Retopo seams already match on both sides.",
            )
            return {"FINISHED"}
        ...
        if not self.apply:
            ...
            msg = f"Would add {shape}. Enable Apply to create them."
            if off_mm > 1.0:
...
class AC9_OT_SyncSelectedVertex(bpy.types.Operator):
    ...
    The targeted counterpart to Sync Retopo Seams: instead of mirroring
    ...
    half. Works from either Edit or Object Mode — it reads the current
    selection, then does the actual work in Object Mode
    """
    ...
    def execute(self, context):
        ...
        unmatched_note = (
            ...
            f"are not on any sewn seam and were left alone."
        ...
        if not self.apply:
            ...
            self.report({"INFO"}, f"Would add {shape}.{unmatched_note} "
        ...
        if res["applied"] is None:
            ...
            self.report({"WARNING" if res["unmatched"] else "INFO"},
                        ...
                        f"already matches.{unmatched_note}")
            return {"FINISHED"}
...
class AC9_OT_SeamStatusReport(bpy.types.Operator):
    ...
    result can be checked rather than taken on trust:
      ...
      align  — how far apart corresponding vertices sit ALONG the seam. This
               ...
               to hold downstream. Blank when the two sides hold different
    ...
    bl_description = (
        "Check every sewn seam on the Guide against the retopo: green = both "
        "sides pair up, red = both sides exist but disagree, orange = one side "
        "only. Colours the seams in the viewport (Seam Status overlay), selects "
        "the vertices on the problem seams and writes the full table to the "
        "Text 'AC9_SeamStatus'"
    )
    bl_options = {"REGISTER", "UNDO"}
    ...
    select_problems: bpy.props.BoolProperty(
        ...
        description=(
            ...
            "authored on one side only, so they can be found in the viewport "
    ...
    def execute(self, context):
        ...
        try:
            ...
            rows, summary = rss.classify_seam_pairs(
                guide, retopo, flat_sk, tol=props.retopo_sync_tol
            )
        ...
        problem_states = (rss.STATE_MISMATCH, rss.STATE_ONE_SIDED)
        ...
        drawn = {s: (ov._batches[f"status_{s}"] is not None)
                 for s in ("matched", "mismatch", "one_sided")}
        ...
        lines = [
            ...
            f"  mismatch            : {summary[rss.STATE_MISMATCH]}",
        ...
        msg = (f"{summary[rss.STATE_MATCHED]} matched, "
               f"{summary[rss.STATE_MISMATCH]} mismatched, "
        ...
        top.status_boundary = (
            ...
            f"{summary[rss.STATE_MISMATCH]} bad / "
            f"{summary[rss.STATE_ONE_SIDED]} half"
        )
        level = "WARNING" if summary[rss.STATE_MISMATCH] else "INFO"
        self.report({level}, msg)
        return {"FINISHED"}
...
class AC9_OT_GenerateSeamChain(bpy.types.Operator):
    ...
    Sync Retopo Seams can only mirror what one side already has, so a seam
...
class AC9_OT_PreviewFill(Operator):
    """Fill every panel with a throwaway quad mesh, to judge the boundary density
    ...
    def execute(self, context):
        ...
        top.status_boundary = (f"Preview: {rep['regions']} panel(s), {quads} quads / "
                               f"{tris} tris at {target:.0f}mm")
        note = ""
        if rep["skipped"]:
            note = (f" {rep['skipped']} panel(s) skipped — their outline does "
                    f"not close.")
        self.report({"INFO"},
                    f"Filled {rep['regions']} panel(s): {quads} quads, "
    ...
    def execute(self, context):
        ...
        try:
            for index, (pos_a, pos_b) in enumerate(pairs):
                wm.progress_update(int(100.0 * index / max(1, len(pairs))))
                bm = bmesh.from_edit_mesh(mesh)
                bm.verts.ensure_lookup_table()
