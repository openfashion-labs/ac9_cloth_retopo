...
| **Inset Pieces** | 型紙ごとに、平面シェイプキー上で外周を **Width** だけ内側へオフセットし、そのあいだを三角形で張り直して、縫い目も自由辺も区別なく平行な頂点列を作る。頂点を溶接しないので外周の頂点は消えず、タグ済みの折れ線は制約として残る | Guide が Object Mode / 平面シェイプキー必須 | 結果行に型紙数・張り直した面数・新しい行の頂点数・帯と隙間の面数・制約として残した折れ辺の数、そして desync が出る |
| **Inset Line** | 折れ線の左右 **Width** の帯を三角形で張り直す。折れ線自身の頂点と辺はその場に残る。**選択した辺**を使い（つながった 1 本が 1 つの線）、何も選択していなければ **Find Folds** のタグ辺を使う。帯は **Inset Pieces** が残した行で止まる | Guide が Edit Mode / 平面シェイプキー必須 | 帯ができる。結果行に処理した線の本数（分岐・短すぎた数つき）・帯の面数・行の辺数、そして desync が出る |
| **Width** | インセットの半幅（メッシュ単位、既定 0.001 = 1 mm）。この幅の中にあった元のジオメトリは新しい三角形に置き換わる。大きいほど陰影のグラデーションが緩くなる | — | 上の 2 つのインセットに効く |
