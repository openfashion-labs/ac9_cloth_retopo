...
| **Levels** / **Preview** | runs the real Subdivide, outline snap, and Guide reprojection for the chosen number of Levels on a disposable copy, then shows that result on the Mirror | Object Mode / 2D state | the Retopo stays unchanged at low resolution. Press Preview again to return to the ordinary Mirror. Editing the Retopo or Guide while it is shown changes the button to **Preview (stale)**; press it to recompute. Refresh Mirror also recomputes a displayed Preview. No preview cache is saved |
| **Commit → Subdivide** | uses the Levels shared with Preview to subdivide the Retopo in 2D, snap new boundary vertices onto the Guide's sewn-seam lines, and re-project into 3D | Object Mode / only while in 2D state (doesn't work while `AC9_3D_Project` is being displayed) | destructive, permanently raises the resolution, and turns Preview off once. New vertices are projected onto the Guide surface, so no smoothing pass is needed. The ordinary Mirror updates automatically if it exists. Without seam analysis it still runs, but skips snapping (with an info-row warning). N-gons (faces with more than four vertices) pass through undivided — only their count is reported on the result row (this is how Blender's Subdivide works; see 06. Troubleshooting) |
