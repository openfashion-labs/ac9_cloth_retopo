...
**赤十字は「対岸」に出ます（Orphan Rings）。** 未配置ゴーストの赤十字が立つのは、頂点が足りない **相手のパネル** の上です。分割された平面レイアウトでは、それは今見ている頂点からパネル1枚ぶん離れた場所で、作業中の拡大率ではまず画面の外にあります。つまり「相棒が居ない頂点」を選んでいても、その頂点のところには何も出ません。**Orphan Rings**（**Points** が ON のとき有効）は、その未配置ゴーストを **持っている側** の頂点にリングを描きます。色は赤十字と同じ、形はリングなので、どちらが頂点でどちらが空き位置かが区別できます。対象は縫い目のゴーストだけです（フリー辺の足は頂点自身のすぐ横にあるので、リングを足すと十字と重なって潰れます）。

リングの意味は「この頂点の対岸の枠が空いている」であって、「この頂点が間違っている」ではありません。縫い目の片側が3頂点・反対側が2頂点なら、**Bond Distance** 内の最近傍マッチで2組が成立し、残った1頂点にリングが付きます。どれが余るかは並び方しだいです。

**Lines** を ON にすると、その頂点と赤十字を結ぶ線が引かれます。既定では **Unplaced Lines Only** も ON で、未配置ゴーストの線だけが出ます。配置済みゴーストの線は「既にある頂点の上に乗っている」という確認でしかなく、仕上がったパネルでは未配置のものより2桁多いので、全部引くと肝心の数本が埋もれます。
...
**なぜ隙間ができるのか。** Finalize は頂点を 1 個ずつ独立に Guide へ射影します。縫い目の両側にある 2 頂点は布の上では同じ場所ですが、別々に射影されるので別の点に着地します。原因はリトポ側ではなく Guide 側です。CLO は縫い合わせた辺を同じ座標にしません。実測（パンツの Guide、縫い目ペア 400 組）で両側が最大 2.061mm・平均 0.941mm 離れており、リトポはそれを忠実に再現しています。結果として Finalize の出力には、実測で最大 0.857mm・平均 0.066mm・0.1mm 超が 170 組中 33 組の割れが残っていました。

**Close Seam Gaps**（既定 ON）は、対になった両側をその中点へ寄せます。動くのは位置だけで、頂点数も面数も変わりません。1 頂点が動く距離は閉じる隙間の半分までです（上の例で最大 0.428mm）。対象は Ghost が配置済み（緑）のペアだけです。未配置（赤）の Ghost には合わせる相手が居ないので、そのままにして件数を結果行に出します。重ね縫い（Mark Sharp で指定したポケット等）は対象外です。あれは表面に乗った別レイヤーで、3D では本当に離れています。
...
| **Drape → Bake** | Guide の **3D 形状**から Ambient Occlusion と Curvature（**Curvature Radius** で決まるスケールの凹凸）を焼き、平面レイアウトに載せて、さらに掛け合わせる。ベイクの間だけ Flat SK を一時的に外し、終わったら戻す | Object Mode / Guide | 画像 `AC9_DrapeMap_<Guide名>`。2D でナイフを入れるときの当たりに使う。AO と Curvature のパスは合成が終わると削除される（残したいときは **Keep Passes**） |
...
| **Plane** | Flat SK 空間に 1×1 m のプレーン `AC9_BakePreview` を作る（あれば再利用）。Emission マテリアルなので Image Editor 無しで見られる | Object Mode | Solid シェーディングのビューポートを **Solid の色 = Texture** に切り替え、**X-Ray と Retopology オーバーレイを OFF に戻す**（どちらもこのツールが ON にするものではない。理由は下記）。Material Preview / Rendered のビューポートは触らない。**プレーンは z = 0 の 5mm 下**にあるので、z = 0 に平らに置かれている物（平面状態の Guide、平面リトポ、自作のベイク板）が上から見たときにプレーンを隠す。リトポは切っている本体なので隠せない — それには **Add Transparent Material** を使う。残りは**手で隠す**こと（このボタンは他オブジェクトの表示を触らない） |
| **Add Transparent Material** | Retopo Mesh に半透明の作業用マテリアル `AC9_RetopoTransparent` を付ける。5mm 下の Preview Plane が、いま切っている面越しに読めるようになる | Object Mode / Retopo | ファイルに `AC9_RetopoTransparent`（旧名 `AC9_RetopoGhost`）が既にあれば**それを再利用**し、設定済みのアルファをそのまま使う。**リトポが他のマテリアルを持っている場合は拒否して止まる** — どのスロットが何かはユーザーの領分で、並べ替えはこのツールの仕事ではない。あわせて **Retopology オーバーレイを全ワークスペースで OFF** にする（そのオーバーレイは半透明マテリアルの透明度を上書きしてしまうため。**Plane** は見ているワークスペースしか触らないので、こちらが広い）。付いている間はこの行が **Alpha** スライダーと解除ボタン（×）に変わる |
| **Alpha** | リトポの透け具合（0 = 完全に透明、1 = 不透明。既定 0.35） | 上のマテリアルが付いているとき | 値は `AC9_RetopoTransparent` マテリアル自身が持つので **.blend に保存**され、**プリファレンスには一切触れない**。Solid と Material Preview は透明度を別々のプロパティから読む（実測）ので、このスライダーは両方に書き込む |
...
**Retopology オーバーレイを使わない理由**: あのオーバーレイは編集中メッシュの面の塗りをテーマの `face_retopology` 色（実測でアルファ 0.502）に差し替えます。つまり **ON の間は Edit Mode で半透明マテリアルの透明度が効きません** — リトポはテーマが決めた濃さより薄くできず、そのテーマは**全ファイル共通のプリファレンス**です。さらに `show_retopology` は 3D ビューごとに **.blend の中へ保存される**ので（実測: 保存して開き直しても残る）、過去バージョンがこれを ON にしたファイルは今もそれを抱えています。だから **Plane** は、X-Ray と同じように、見つけたら OFF に戻します。
...
| **Curvature Radius** | Curvature が拾う凹凸のスケール（mm、既定 8）。この幅で Guide を平滑化し、平滑化した形から法線方向にどれだけ浮いているかを出す。**白＝凸（稜線）・黒＝凹（折り目）・中間のグレー＝平ら**。半径よりずっと広い起伏は基準ごと平滑化されて消えるので、切りたい稜線の幅くらいに合わせる。小さくすると布の織り目まで拾って粒状になり、大きくすると隣り合う皺が溶け合う。コストは半径の2乗で増える（実測 172k 頂点: 8mm で 0.98 秒、20mm で 5.4 秒。2K のベイク自体が 2.7 秒） |
