...
def find_vertex_partners(guide_obj, match_distance=0.0, precision_3d=4):
    ...
    return (boundary_verts, vert_neighbors,
            drop_isolated_contacts(boundary_verts, vert_neighbors, dict(partners)))


# Longest run of unmatched boundary vertices that find_anchors still reads as
# a dropout INSIDE a seam rather than a free edge (see its docstring).
MAX_DROPOUT = 3


def _matched_within_reach(v, nb, partners, vert_neighbors):
    """Walking away from v through nb, is there a matched vertex within
    MAX_DROPOUT + 1 boundary steps? The first step IS nb itself.

    Same walk as _reaches_through_dropouts: straight along the boundary, and
    it gives up at an irregular vertex, where "along the boundary" stops
    meaning one thing.
    """
    prev, cur = v, nb
    for _ in range(MAX_DROPOUT + 1):
        if partners.get(cur):
            return True
        nbs = vert_neighbors.get(cur, ())
        if len(nbs) != 2:
            return False
        prev, cur = cur, (nbs[0] if nbs[0] != prev else nbs[1])
    return False


def drop_isolated_contacts(boundary_verts, vert_neighbors, partners):
    """Forget partnerships that exist at ONE point and nowhere near it.

    A partnership means "sewn to". Sewing runs along a length, so a genuine
    seam vertex has company: its own neighbours are matched too, or at worst
    a short dropout separates it from the next matched vertex (MAX_DROPOUT —
    see find_anchors). A boundary vertex that is matched while nothing within
    MAX_DROPOUT + 1 steps either way is matched is not sewn to anything. It
    is a free edge that happens to TOUCH another panel at one point, which
    CLO exports all the time: a hem brushing a pocket, a lapel tip landing on
    the body, two outlines crossing after Inset Pieces welded a needle away.

    Left in, find_anchors reads that single point as a seam and puts an
    anchor at BOTH of its ends — two anchors, chopping the free run it sits
    in into three, for a contact that carries no correspondence at all.

    So: for every such isolated contact v, drop v's partners, and drop v from
    its partners' sets. The second half matters even when the partner is a
    real seam vertex on its own side (the usual case — a free tip touching
    mid-seam): there, v is a stray extra partner that rule (c) would read as
    a panel starting or stopping, so it would raise an anchor on the OTHER
    side too. Nothing else about that partner changes; its own seam is intact.

    Applied at the end of find_vertex_partners so every consumer — Generate,
    Status, Density, Sync, the Anchors overlay — sees the same partners.

    What this deliberately does NOT touch: a seam that skips. matched,
    unmatched x k, matched is still a seam while k <= MAX_DROPOUT, because
    the walk reaches the next matched vertex within MAX_DROPOUT + 1 steps.
    Only a contact alone in its whole neighbourhood is dropped.
    """
    isolated = set()
    for v in boundary_verts:
        if not partners.get(v):
            continue
        nbs = vert_neighbors.get(v, ())
        if len(nbs) != 2:
            # Irregular boundary: an anchor in its own right (a walk cannot
            # pass through it), and "both sides of v" is not well defined.
            continue
        if any(_matched_within_reach(v, nb, partners, vert_neighbors)
               for nb in nbs):
            continue
        isolated.add(v)
...
def _reaches_through_dropouts(w, target, partners, vert_neighbors, near=None):
    """Does w's strand continue alongside `target`'s? A walk of at most
    MAX_DROPOUT + 1 boundary steps from w that

      * lands in `target`                                   -> yes
      * lands on a matched vertex whose partners touch `near` (the target's
        own one-step neighbourhood) -> yes: w's strand skipped one vertex
        against the others (a three-layer seam where one layer lost a vertex
        — the layers still run together, one step out of phase)
      * lands on any other matched vertex                   -> no: sewn to
        something else, correspondence really changes here
      * steps across an unmatched vertex (a dropout)        -> keep walking
    """
    if w in target:
        return True
    near = near if near is not None else target
    nbs = vert_neighbors.get(w, ())
    for start in nbs:
        prev, cur = w, start
        for _ in range(MAX_DROPOUT + 1):
            if cur in target:
            ...
            pc = partners.get(cur)
            if pc:
                if any(x in near for x in pc):
                    return True
                break                       # matched to something else
            cnbs = vert_neighbors.get(cur, ())
            if len(cnbs) != 2:
                break
            prev, cur = cur, (cnbs[0] if cnbs[0] != prev else cnbs[1])
    return False


def _one_step_hood(vs, vert_neighbors):
    out = set(vs)
    for c in vs:
        out.update(vert_neighbors.get(c, ()))
    return out


def _seam_continues_past(v, nb, p, partners, vert_neighbors):
    """v is matched, its boundary neighbour nb is not. True when nb starts a
    run of at most MAX_DROPOUT unmatched vertices after which a matched
    vertex m follows, and every partner of v reaches a partner of m across
    dropouts only — the seam carries on, and the run is a vertex one side
    has and the other lacks."""
    prev, cur, run = v, nb, 0
    while run < MAX_DROPOUT:
        run += 1
        nbs = vert_neighbors.get(cur, ())
        if len(nbs) != 2:
            return False
        ...
        if pm:
            target = set(pm)
            near = _one_step_hood(target | {nxt}, vert_neighbors)
            return all(_reaches_through_dropouts(w, target, partners, vert_neighbors, near)
                       for w in p)
        prev, cur = cur, nxt
    return False
...
def find_anchors(boundary_verts, vert_neighbors, partners):
    ...
    Dropouts inside a seam (the one exception to (b))
    --------------------------------------------------
    A short run of unmatched vertices — up to MAX_DROPOUT — between two
    matched ones is NOT a seam end when the seam demonstrably continues past
    it: every partner of v reaches a partner of the first matched vertex
    beyond the run on its own side, stepping across unmatched vertices only
    (_reaches_through_dropouts) — and (c)'s "next to one" test steps across
    dropouts the same way, since the lone vertex may sit on either side.
    Such a run is one side of the seam carrying a vertex the other side does
    not: CLO already exports a few (16 lone ones on a jacket, 4 on pants),
    and CLO Cleanup's Inset Pieces welds outline needles away and makes many
    more (135 on a legwear export). Read as seam ends, each of them put two
    anchors into the seam — anchors 37 -> 231 on that legwear after the
    inset, the Boundary overlay a field of crosses. The span interior never
    had to pair vertex by vertex, so absorbing the dropout costs nothing.
    A genuine seam end is untouched: beyond it the whole free edge is
    unmatched, so no matched vertex is reached within MAX_DROPOUT steps.
    """
    anchors = set()
    for v in boundary_verts:
        p = partners.get(v, ())
        ...
        for nb in nbs:
            pnb = partners.get(nb, ())
            if not pnb:
                if _seam_continues_past(v, nb, p, partners, vert_neighbors):
                    continue            # a dropout inside the seam, not its end
                anchors.add(v)          # (b)
                break
            ...
            carried = set(pnb)
            # "next to one" may step across dropouts (unmatched vertices) on
            # the partner side too — the other half of the dropout rule: when
            # side B keeps a vertex side A lost, A's two neighbours are
            # adjacent while B's partners have the lone vertex between them.
            near = _one_step_hood(carried | {nb}, vert_neighbors)
            if not all(_reaches_through_dropouts(w, carried, partners, vert_neighbors, near)
                       for w in p):
                anchors.add(v)          # (c)
                break
    return anchors
...
def _flat_at_u(span, flat_co, cum, u):
    """Flat-space position at 3D-arc-length parameter u along the span."""
    seg, t = _point_at_arc(span, flat_co, cum, u * cum[-1])
    return flat_co[span[seg]].lerp(flat_co[span[seg + 1]], t)
...
def drop_straight_interior(us, sides, flat_co, tol_mm):
    """Remove the interior division points of every stretch that is straight.

    `us` is one stretch's uniform grid (sorted 0..1 parameters, endpoints
    included); `sides` is a list of (span, cum) — the span and its 3D
    cumulative arc lengths — one entry for a free edge, two for a sewn seam
    (the partner oriented the same way, so the same u names the same sewn
    point on both).

    A window of grid points [i, j] (j - i >= 2, so at least one interior
    point goes) is straight when, on EVERY side, the Guide's flat outline
    between u_i and u_j stays within tol_mm of the chord joining the window's
    two ends. Windows are grown from each start until the first failure,
    then taken longest first without overlap; the interior points of the
    taken windows are dropped.

    Why the window has to span two cells: a tight curve is locally flat
    over ONE spacing cell by definition of a sensible spacing, so a
    single-cell test would call every curve straight. Requiring the chord to
    hold over two cells means a curve is left at Spacing unless its sagitta
    over 2 x Spacing is under tol_mm — i.e. it is a curve nobody would draw
    a vertex on. No separate "minimum straight length" setting needed.

    Why the chord test rather than a turning angle at each Guide vertex:
    CLO's outline is a polyline of particles, and the angle at one particle
    is noise-level on a gentle curve and zero-level on a straight; the
    deviation over a whole chord is what the eye (and the retopo edge that
    will be drawn there) actually sees.

    Returns (kept_us, dropped_count).
    """
    n = len(us)
    if n <= 2 or tol_mm <= 0.0 or flat_co is None:
        return list(us), 0
    tol = tol_mm / 1000.0
    # Per side: the grid points' flat positions, and the Guide vertices that
    # fall strictly inside the stretch with their own u.
    side_data = []
    for span, cum in sides:
        total = cum[-1]
        if total <= 1e-12:
            continue
        grid = [_flat_at_u(span, flat_co, cum, u) for u in us]
        inner = []
        for k in range(1, len(span) - 1):
            uk = cum[k] / total
            if us[0] < uk < us[-1]:
                inner.append((uk, flat_co[span[k]]))
        side_data.append((grid, inner))
    if not side_data:
        return list(us), 0

    def straight(i, j):
        for grid, inner in side_data:
            a, b = grid[i], grid[j]
            for uk, p in inner:
                if us[i] < uk < us[j] and _dist_to_segment(p, a, b) > tol:
                    return False
            # The dropped grid points themselves sit on the outline too.
            for k in range(i + 1, j):
                if _dist_to_segment(grid[k], a, b) > tol:
                    return False
        return True
    ...
    for i in range(n - 2):
        best = None
        j = i + 2
        while j < n and straight(i, j):
            best = j
            j += 1
        if best is not None:
            windows.append((i, best))
    windows.sort(key=lambda w: (w[0] - w[1], w[0]))
    keep = [True] * n
    taken = []
    for a, b in windows:
        if any(a < d and c < b for c, d in taken):
            continue
        taken.append((a, b))
        for k in range(a + 1, b):
            keep[k] = False
    kept = [u for u, k in zip(us, keep) if k]
...
def division_parameters(span, basis_co, hard_verts=(), hard_us=(), count=8,
                        ...
                        flat_co=None, straight_tol_mm=0.0, partner=None,
                        stats=None):
    """Where to divide a span (or a merged run), as parameters 0..1 by 3D
    arc length.

    straight_tol_mm > 0 (Spacing mode only) leaves a stretch of the outline
    undivided wherever it is straight — within that many millimetres of a
    chord in the FLAT layout, over at least two spacing cells — while the
    curved parts keep their Spacing. `flat_co` is required for that;
    `partner` is the seam's other side (oriented like `span`), and a stretch
    then counts as straight only when BOTH sides are: a straight side sewn to
    a curved one (sleeve cap to armhole) still has to follow the curve. See
    drop_straight_interior. `stats["straight_dropped"]` is incremented by
    the number of division points this removed, when a dict is given.
    ...
    for k in range(len(breaks) - 1):
        ...
        if seg_mm < min_seam_mm:
            ...
            n = max(2, int(count))
        stretch = [u0 + (u1 - u0) * i / (n - 1) for i in range(n)]
        if (straight_tol_mm > 0.0 and spacing_mm > 0.0 and n > 2
                and flat_co is not None):
            sides = [(span, cum)]
            if partner is not None and len(partner) >= 2:
                sides.append((partner, _arc_lengths(partner, basis_co)))
            stretch, dropped = drop_straight_interior(
                stretch, sides, flat_co, straight_tol_mm)
            if stats is not None and dropped:
                stats["straight_dropped"] = (
                    stats.get("straight_dropped", 0) + dropped)
        us.extend(stretch)
