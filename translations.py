...
_OPERATOR_LABELS = {
    ...
    "Bake Drape Map": "Drape Map を Bake",
    ...
    "Mark Pin": "Pin をマーク",
    "Merge Drape into Grid": "Drape を Grid へ統合",
...
_UI_STRINGS = {
    ...
    "Drape Merge": "Drape Merge",
    "Face Settings": "Faces 設定",
    "Faces": "Faces",
    "Guide Maps": "Guide Maps",
    "Map Settings": "Map 設定",
    "Merge Settings": "Merge 設定",
    "Prepare": "Prepare",
    "Prepare Settings": "Prepare 設定",
    ...
    "Contact": "接触",
    "Corner": "Corner",
    "Create Flat SK: every selected mesh. Steps 2-4 need it; Basis is left untouched":
        "Create Flat SK は選択中の全メッシュに実行。手順 2〜4 に必要で、Basis は変更しません",
    "Density": "密度",
    ...
    "Hide Sources After Merge": "Merge 後に元を隠す",
    ...
    "Show Other Mode Tools": "別モードのツールも表示",
    "Show Outline": "輪郭を表示",
    ...
    "2048": "2048",
    "2D seam boundary work: seam guides, selected-pair ghost, parity counts, status":
        "2D の Seam Boundary 作業向け: Seam ガイド・選択ペアの Ghost・頂点数の一致・ステータス",
    ...
    "Fan-triangulate each crossed cell. Quads + tris only, no n-gons, but lots of triangles and the odd sliver":
        "交差したセルを扇状に三角化する。クワッドと三角のみで n-gon は出ないが、三角形が多くなり細片も混じる",
    ...
    "Keep each crossed grid cell as a single face (n-gon). Cleanest, easiest to hand-edit, fewest holes — but introduces n-gons":
        "交差した Grid セルを 1 面（n-gon）のまま残す。最もきれいで手編集しやすく穴も少ないが、n-gon が生じる",
    "Look at the Mirror in 3D: white outline and boundary flags":
        "3D で Mirror を見る用: 白い輪郭と Boundary フラグ",
    "Magenta": "マゼンタ",
    "Mirror": "Mirror",
    "N-gon (pure merge)": "N-gon（そのまま統合）",
    ...
    "Quad Fill": "Quad Fill",
    "Read the guide's seam structure: seams, pair lines, folds, twins, white outline, ghosts":
        "Guide の Seam 構造を読む用: Seams・ペア線・折れ線・Twin・白い輪郭・Ghost",
    "Red": "赤",
    "Refill the seam band with quad-dominant topology: crossed cells are removed whole, the band is Delaunay-triangulated from its boundary verts and the triangles are paired into quads. Closest to a hand-made seam; a few triangles remain where parity forces them":
        "Seam の帯をクワッド優勢のトポロジーで貼り直す。交差セルは丸ごと取り除き、帯を外周頂点から Delaunay 三角化して三角形をクワッドに対にする。手作業の Seam に最も近いが、頂点数の都合で三角形が少し残る",
    ...
    "Show the Drape Map (AC9_DrapeMap)": "Drape Map (AC9_DrapeMap) を表示する",
    "Show the Residual Map (AC9_ResidualMap)": "Residual Map (AC9_ResidualMap) を表示する",
    "Show the Sag Map (AC9_SagMap)": "Sag Map (AC9_SagMap) を表示する",
    "Spacing": "間隔",
    ...
    "Create Flat SK runs in Object Mode.": "Create Flat SK は Object Mode で実行してください。",
    ...
    "No Mirror found — run Refresh Mirror first.":
        "Mirror がありません — 先に Refresh Mirror を実行してください。",
    ...
    "Select at least one mesh.": "メッシュを 1 つ以上選択してください。",
    ...
    "Set the Drape and Grid objects first.": "先に Drape と Grid のオブジェクトを設定してください。",
    "Set the Guide and Flat SK first.": "先に Guide と Flat SK を設定してください。",
    ...
    "Switch Guide to 3D mode first.": "先に Guide を 3D モードに切り替えてください。",
    ...
    "Check every sewn seam on the Guide against the retopo: green = both sides pair up, red = both sides exist but disagree, orange = one side only. Colours the seams in the viewport (Seam Status overlay), selects the vertices on the problem seams and writes the full table to the Text 'AC9_SeamStatus'":
        "Guide 上のすべての縫い Seam を retopo と照合します: 緑 = 両側がペアになっている、赤 = 両側あるが食い違っている、オレンジ = 片側だけ。ビューポートで Seam に色を付け（Seam ステータス Overlay）、問題のある Seam の頂点を選択し、全一覧をテキスト 'AC9_SeamStatus' に書き出します",
    ...
    "Embed the Drape strips into the Grid: clip the grid cells the drape crosses (drape wins) and fill the seam, then stitch into one mesh":
        "Drape の帯を Grid に埋め込みます: Drape が横切る Grid セルを切り抜き（Drape 優先）、Seam を埋めて 1 つのメッシュに縫い合わせます",
    "Enter Edit Mode and select interior boundary loops (holes) on the active mesh. The outer outline is ignored. Nothing selected = no holes":
        "Edit Mode に入り、アクティブメッシュの内側の外周ループ（穴）を選択します。外側の輪郭は無視されます。何も選択されなければ穴はありません",
    ...
    "Move selected retopo vertices; snaps only to ghost points when within Snap Distance. Activated by G when Ghost Snap Mode is ON":
        "選択した retopo 頂点を移動します。Snap Distance 以内の Ghost 点にだけスナップします。Ghost Snap Mode が ON のとき G で起動します",
    ...
    "Snap each SELECTED retopo vertex exactly onto its nearest ghost when within Snap Distance (the same radius as Ghost Snap on G). Non-destructive: selected verts with no ghost in range — and all unselected verts — are left untouched. Use to finish seams you placed roughly but forgot to snap exactly":
        "選択した各 retopo 頂点を、Snap Distance 以内にある最も近い Ghost にぴったりスナップします（G の Ghost Snap と同じ半径）。非破壊的です: 範囲内に Ghost がない選択頂点と、選択されていない頂点はすべて手つかずです。だいたいの位置に置いたまま正確なスナップを忘れた Seam を仕上げるのに使います",
    ...
    "Toggle the Guide between 2D flat layout (work mode) and 3D garment shape (reference mode).\n→ 3D: sets Flat SK to 0, unhides the guide (including parent collections), turns off Wireframe overlay in PERSPECTIVE viewports only (orthographic / 2D work views are untouched).\n→ 2D: restores Flat SK to 1, guide + collection visibility, and wireframe state":
        "Guide を 2D 平面レイアウト（作業モード）と 3D 衣装形状（参照モード）で切り替えます。\n→ 3D: Flat SK を 0 にし、Guide を（親コレクションも含めて）表示し、透視投影のビューポートに限りワイヤーフレーム Overlay を OFF にします（平行投影／2D 作業ビューは触りません）。\n→ 2D: Flat SK を 1 に戻し、Guide とコレクションの表示状態、ワイヤーフレーム状態を復元します",
    "Toggle which is visible: the Guide (high-poly garment in 3D) or the Mirror (retopo projected onto the garment). Only active when the Guide is in 3D (Flat SK = 0) and a Mirror exists":
        "どちらを表示するかを切り替えます: Guide（3D のハイポリ衣装）か Mirror（衣装に投影した retopo）。Guide が 3D（Flat SK = 0）で、かつ Mirror が存在するときだけ有効です",
    ...
    "Also show the Boundary tools of the other mode (they run only in their own mode)":
        "別モードの Boundary ツールも表示します（実行はそれぞれのモードでのみ可能）",
    "Also show the Faces tools of the other mode (they run only in their own mode)":
        "別モードの Faces ツールも表示します（実行はそれぞれのモードでのみ可能）",
    ...
    "Automatically run 'Refresh Mirror' whenever you leave Edit Mode on the 2D retopo. Skipped (with a warning) if the mirror currently holds un-applied moves from 'Apply 3D Edits > 2D' — refreshing would silently discard them. Press Refresh manually in that case":
        "2D retopo の Edit Mode を抜けるたびに 'Refresh Mirror' を自動実行します。Mirror に 'Apply 3D Edits > 2D' 未適用の移動が残っている場合は（警告付きで）スキップします — 更新すると黙って破棄されてしまうためです。その場合は手動で Refresh を押してください",
    ...
    "Cell size for the structured grid. 0 follows the boundary Spacing above":
        "構造グリッドのセルサイズ。0 なら上の外周 Spacing に従います",
    ...
    "Coincident verts closer than this are welded when stitching the three pieces (grid / drape / seam band) into one mesh":
        "3 つのピース（Grid / Drape / Seam の帯）を 1 つのメッシュに縫い合わせるとき、これより近い重複頂点を溶接します",
    "Collapse sliver triangles whose short edge is below this fraction of the average grid cell edge. Removes the thin black wedges along the seam. 0 = off. Too high will eat real detail":
        "短辺が Grid セル平均辺長のこの比率より小さい細片三角形を collapse します。Seam 沿いの細い黒いくさびが消えます。0 = 無効。大きすぎると本来のディテールまで食います",
    "Colour each sewn seam by whether the retopo pairs up on it: green = both sides authored and aligned, red = both sides but they disagree, orange = only one side done. Seams with no retopo yet are left uncoloured. Run 'Seam Status' to fill it in":
        "各縫い Seam を、retopo がそこでペアになっているかで色付けします: 緑 = 両側が作られてそろっている、赤 = 両側あるが食い違っている、オレンジ = 片側だけ完了。retopo が未着手の Seam は色が付きません。'Seam Status' を実行すると埋まります",
    ...
    "Draw the Guide's full pattern outline (every UV seam edge: sewn seams and free edges alike) as white lines over the Guide mesh. Always on top, so it stays visible in Solid mode, and follows the Guide's current ShapeKey blend (2D ↔ 3D). The cyan Seams overlay shows only the sewn pairs":
        "Guide の型紙輪郭全体（縫い Seam もフリー辺も含む、すべての UV シーム辺）を Guide メッシュの上に白い線で描きます。常に最前面なので Solid モードでも見え、Guide の現在の ShapeKey ブレンド（2D ↔ 3D）に追従します。シアンの Seams Overlay は縫いペアだけを表示します",
    ...
    "Flat quad-grid mesh (the clean uniform layout). Cells the Drape crosses are clipped/filled; the rest is preserved as quads. Both meshes must be flat (z = 0) and in the same world space":
        "平面のクワッドグリッドメッシュ（きれいな均一レイアウト）。Drape が横切るセルは切り抜き／埋められ、残りはクワッドのまま保たれます。両方のメッシュが平面 (z = 0) で、同じワールド空間にある必要があります",
    "Flat strip mesh following the drape (wrinkle) lines. Kept whole — the drape flow has priority and is embedded into the Grid":
        "ドレープ（しわ）の流れに沿った平面の帯メッシュ。丸ごと保たれます — ドレープの流れが優先され、Grid に埋め込まれます",
    "Grow the Drape footprint by this distance before clipping the Grid. Larger removes a wider strip of grid around the drape (cleaner seam, fewer slivers). 0 = clip the grid exactly at the drape edge":
        "Grid を切り抜く前に、Drape の占有範囲をこの距離だけ広げます。大きくすると Drape のまわりの Grid をより広く取り除きます（Seam がきれいになり細片が減ります）。0 = Drape の縁でちょうど Grid を切り抜きます",
    "Half-width of the density-pin squares (flat-layout world units)":
        "密度 Pin の四角マーカーの半幅（平面レイアウトのワールド単位）",
    "Half-width of the inset (mesh units): the new vertex row is placed this far in from the outline (Inset Pieces) or on each side of a fold line (Inset Line), and every original vertex closer than this is absorbed first. Larger = softer shading gradient":
        "インセットの半幅（メッシュ単位）: 新しい頂点列は輪郭からこの距離だけ内側（Inset Pieces）または折れ線の両側（Inset Line）に置かれ、これより近い元の頂点はすべて先に吸収されます。大きくするとシェーディングのグラデーションが緩やかになります",
    "Half-width of the topology-corner diamonds (flat-layout world units)":
        "トポロジー Corner のひし形マーカーの半幅（平面レイアウトのワールド単位）",
    "Hide the Drape and Grid source objects once the merge succeeds":
        "Merge が成功したら Drape と Grid の元オブジェクトを隠します",
    "How close a retopo boundary vertex must be to a Guide seam to count as sitting on it. Vertices further away are cuts through the middle of a panel and are left alone":
        "retopo の外周頂点が Guide の Seam 上にあるとみなすための近さ。これより遠い頂点は型紙の途中を通るカットとして手つかずになります",
    "How close a retopo vertex must be to a ghost to count as BONDED (placed → green). Separate from Snap Distance, which is only the interactive Ghost Snap drag radius. Keep this small (a few mm): it is also the radius Force Bond uses to pull selected verts exactly onto their ghost, so a large value would yank unrelated verts onto the seam and distort faces":
        "retopo 頂点が Ghost に BONDED（配置済み → 緑）とみなされるための近さ。対話的な Ghost Snap のドラッグ半径にすぎない Snap Distance とは別物です。小さめ（数 mm）に保ってください: これは Force Bond が選択頂点を Ghost の上へ正確に引き寄せる半径でもあるので、大きすぎると無関係な頂点まで Seam に引っ張って面を歪めます",
    ...
    "How far (mm, in the 2D flat layout) a Guide vertex may sit from the retopo's 2D footprint and still count as covered. Outside this the residual map shows neutral dark gray":
        "Guide の頂点が retopo の 2D 占有範囲からどれだけ離れていても被覆済みとみなすか（2D 平面レイアウト上の mm）。これを超えると Residual Map は中立の濃いグレーになります",
    ...
    "How to fill a grid cell that the drape strips cross":
        "Drape の帯が横切る Grid セルをどう埋めるか",
    ...
    "Interior quad size for the preview fill. 0 follows the boundary Spacing above, which is what makes the preview honest: the interior then shows the density the boundary is asking for":
        "Preview Fill の内部クワッドサイズ。0 なら上の外周 Spacing に従い、それがプレビューを正直なものにします: 内部は外周が要求している密度をそのまま示します",
    ...
    "Max distance a new boundary vert may move to reach a seam line":
        "新しい外周頂点が Seam 線に届くために移動できる最大距離",
    ...
    "Max shape distortion allowed when pairing two seam triangles into a quad (Quad Fill mode). Higher pairs more triangles but allows more skewed quads; lower leaves more triangles in the seam":
        "Seam の三角形 2 つをクワッドに対にするとき許容する形状の歪みの上限（Quad Fill モード）。高くするとより多くの三角形が対になりますが歪んだクワッドも許され、低くすると Seam に三角形が多く残ります",
    ...
    "Maximum distance (flat-layout world units, ~metres) for a retopo vertex to be treated as sitting on a seam and given a ghost":
        "retopo 頂点が Seam 上にあるとみなされ Ghost が与えられる距離の上限（平面レイアウトのワールド単位、およそメートル）",
    "Maximum distance (guide-space units) for a moving vertex to snap to a ghost. Enable 'Show Snap Radius' to visualize this as circles around each ghost":
        "移動中の頂点が Ghost にスナップする距離の上限（Guide 空間の単位）。'Show Snap Radius' を有効にすると、各 Ghost のまわりの円として可視化できます",
    ...
    "Name for the merged mesh object": "統合後のメッシュオブジェクトの名前",
    ...
    "Project edges marked Sharp on the GUIDE mesh onto the nearest other panel and show them as seam pairs. Mark a pocket outline with Mark Sharp to register layered seams explicitly. Turn OFF if the guide came in with stray Sharp edges (some importers mark hard edges) and unexpected guides appear":
        "GUIDE メッシュ上で Sharp にマークした辺を最も近い別の型紙に投影し、Seam ペアとして表示します。ポケットの輪郭を Mark Sharp すれば重ね縫いを明示的に登録できます。Guide に余計な Sharp 辺が付いた状態で読み込まれ（ハードエッジをマークするインポーターもあります）意図しないガイドが現れる場合は OFF にしてください",
    "Quad Fill: also remove green cells that come within this fraction of a cell of the drape, so the refilled seam band never gets squeezed thinner than a workable quad. 0 = only cells the drape actually touches give way":
        "Quad Fill: Drape からセルのこの比率以内に入る緑のセルも取り除き、貼り直した Seam の帯が使えるクワッドより細く潰されないようにします。0 = Drape が実際に触れたセルだけが譲ります",
    "Quad Fill: loop-cut drape edges longer than ~1.4 grid cells before merging, so the seam band gets attachment verts at grid density along the whole drape outline (the drape strips themselves are subdivided to match the grid). Off = keep the drape mesh untouched":
        "Quad Fill: 統合の前に Grid セル約 1.4 個より長い Drape の辺をループカットし、Drape の輪郭全体にわたって Seam の帯が Grid 密度の接続頂点を得られるようにします（Drape の帯自体も Grid に合わせて分割されます）。OFF = Drape メッシュを手つかずのまま保ちます",
    ...
    "Roughly how far apart the generated vertices should sit":
        "生成される頂点をおよそどれくらいの間隔で置くか",
    ...
    "Select the retopo vertices on every seam that is mismatched or authored on one side only, so they can be found in the viewport without relying on the overlay":
        "食い違っている Seam、または片側だけ作られている Seam すべての retopo 頂点を選択し、Overlay に頼らずビューポートで見つけられるようにします",
    ...
    "Show unfinished tools: Preview Fill, Grid Regions, Drape Merge, Guide Separate, Mesh Edit, Quad Fix, Legacy Flip":
        "未完成のツールを表示します: Preview Fill, Grid Regions, Drape Merge, Guide Separate, Mesh Edit, Quad Fix, Legacy Flip",
    ...
    "Stitch vertices that sit on the middle of a neighbouring face's edge (the 'fake' verts) by splitting that edge so the vertex is genuinely shared. Makes the mesh watertight — no overlaps/gaps when you move verts. Adds a collinear vertex to the neighbour (so a quad may become a 5-gon)":
        "隣の面の辺の途中に載っている頂点（「偽の」頂点）を、その辺を分割して本当に共有された状態にします。メッシュが水密になり、頂点を動かしても重なりや隙間ができません。隣の面には同一直線上の頂点が 1 つ加わります（クワッドが 5 角形になることがあります）",
    ...
    "Triangulate each mesh before flattening. CLO Projector requires an all-triangle Guide (validate_guide rejects any n-gon or quad), so this is on by default — turn it off only if the mesh is already triangulated and you want to keep its exact topology":
        "平面化の前に各メッシュを三角化します。CLO Projector は全三角形の Guide を要求するため（validate_guide は n-gon やクワッドを拒否します）既定で ON です — メッシュが既に三角化済みで、そのトポロジーを正確に保ちたい場合だけ OFF にしてください",
    ...
    "UV layer to flatten. Empty uses each mesh's active UV layer":
        "平面化する UV レイヤー。空なら各メッシュのアクティブな UV レイヤーを使います",
    ...
    "Which baked map the Preview Plane displays": "プレビュー平面がどのベイク済みマップを表示するか",
