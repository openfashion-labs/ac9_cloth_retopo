...
_state: dict = {
    ...
    # Open-boundary edge list of the Guide, memoised: enumerating it costs
    # ~300ms on a production Guide (see analysis._compute_boundary_mask_and_
    # edges) and the outline rebuilds on every flat-SK slider tick, where only
    # the COORDINATES change. Key = (mesh name, |verts|, |edges|, |polygons|).
    "outline_edges":            None,
    "outline_edges_key":        None,
    "outline_drawn_edges":      0,      # diagnostics: edges in the last batch
}
...
def reset_guide_derived():
    """The Guide pointer was swapped: forget everything about the old one.

    invalidate() alone is not enough here. The outline batch is rebuilt from
    the flat-SK POLL (_state["last_flat_sk_val"]) as well as from the dirty
    flag, and a new Guide whose Flat SK happens to sit at the same value as
    the old one's (1.0 is the normal case) reads as "nothing changed" — so
    the jacket's outline stayed on screen. The memoised boundary-edge list
    goes too: its key is (mesh name, counts), which two different Guides can
    collide on.
    """
    _state["batch_seams"] = None
    _state["dirty"] = True
    _state["last_flat_sk_val"] = -1.0
    _state["outline_edges"] = None
    _state["outline_edges_key"] = None
    _state["outline_drawn_edges"] = 0
    _state["jumped_indices"] = frozenset()
    _tag_redraw_3d()


def reset_retopo_derived():
    """The Retopo pointer was swapped: rebuild the boundary-flag markers from
    the new mesh (the vert-count tracker would otherwise think it already
    matched) and drop the jumped-island warning, which was about the old one."""
...
def _outline_edges(mesh_data):
    """(v0, v1) for every OPEN-BOUNDARY edge of the Guide, memoised.

    Cached on the mesh's element counts: a flat-SK blend moves vertices but
    never changes which edges are on the boundary, and that is the case this
    rebuilds for on every slider tick.
    """
    key = (mesh_data.name, len(mesh_data.vertices),
           len(mesh_data.edges), len(mesh_data.polygons))
    if _state["outline_edges_key"] == key and _state["outline_edges"] is not None:
        return _state["outline_edges"]
    from ..uv_seam_guide.analysis import _compute_boundary_edges
    edges = _compute_boundary_edges(mesh_data)
    _state["outline_edges"] = edges
    _state["outline_edges_key"] = key
    return edges


def _build_seam_batch(mesh_data, verts_world: list):
    """The Guide's pattern outline: every open-boundary edge, plus any edge
    the user flagged as a UV seam.

    Boundary rather than use_seam alone, because a CLO export need not carry
    any use_seam flag at all — measured on a production Guide, 5,133 boundary
    edges and 0 use_seam, so this overlay drew nothing and the free edges were
    invisible in 3D. use_seam edges stay in so a hand-marked interior line
    (a cut the user intends to open) still shows.
    """
    coords = []
    seen = set()
    for v0, v1 in _outline_edges(mesh_data):
        seen.add((v0, v1) if v0 < v1 else (v1, v0))
        coords.append(verts_world[v0])
        coords.append(verts_world[v1])
    for e in mesh_data.edges:
        if not e.use_seam:
            continue
        v0, v1 = e.vertices[0], e.vertices[1]
        if ((v0, v1) if v0 < v1 else (v1, v0)) in seen:
            continue
        coords.append(verts_world[v0])
        coords.append(verts_world[v1])
    _state["outline_drawn_edges"] = len(coords) // 2
    if not coords:
        return None
