...
def detect_mirror_pairs(guide_obj, flat_sk_name, tolerance=0.02,
    ...
    Returns (pairs, n_tested, rejections):
      pairs — list of dicts, one per mutual-best pair inside `tolerance`:
        ...
        same_handed        True when rot_rms < rms, i.e. the two panels look
                           like duplicate copies of one piece rather than a
                           mirrored left/right pair — worth surfacing, since
                           mirroring one onto the other would then be wrong
    ...
    for ra, info in best_for.items():
        ...
        pairs.append({
            ...
            "same_handed": info["rot_rms"] < info["rms"],
