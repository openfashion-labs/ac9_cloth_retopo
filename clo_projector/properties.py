...
class AC9CloProjectorProps(PropertyGroup):
    ...
    live_update: BoolProperty(
        name="Auto-Refresh Mirror",
        description=(
            "Automatically run 'Refresh Mirror' whenever you leave Edit Mode "
            "on the 2D retopo. Skipped (with a warning) if the mirror currently "
            "holds un-applied moves from 'Apply 3D Edits > 2D' — refreshing would "
            "silently discard them. Press Refresh manually in that case"
        ),
        default=False,
    )
    ...
    show_seam_lines: BoolProperty(
        name="Show Outline",
        description=(
            "Draw the Guide's full pattern outline (every UV seam edge: sewn "
            "seams and free edges alike) as white lines over the Guide mesh. "
            "Always on top, so it stays visible in Solid mode, and follows the "
            "Guide's current ShapeKey blend (2D ↔ 3D). The cyan Seams overlay "
            "shows only the sewn pairs"
        ),
        default=True,
        update=_overlay_toggle,
    )
