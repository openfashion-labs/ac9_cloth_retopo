...
| **Subdivide** | simply subdivides the Retopo in 2D, snaps the new boundary vertices onto the Guide's sewn-seam lines, and re-projects into 3D | Object Mode / only while in 2D state (doesn't work while `AC9_3D_Project` is being displayed) | destructive, and permanently raises the resolution. New vertices are also projected onto the Guide surface, so no smoothing pass is needed. The Mirror updates automatically if it exists. Without seam analysis it still runs, but skips snapping (with an info-row warning). N-gons (faces with more than four vertices) pass through undivided — only their count is reported on the result row (this is how Blender's Subdivide works; see 06. Troubleshooting) |
