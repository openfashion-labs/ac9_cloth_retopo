...
| **Plane** | creates (or reuses) a 1×1 m plane `AC9_BakePreview` in Flat SK space. Uses an Emission material so it's visible without the Image Editor | Object Mode | switches a Solid-shading viewport to **Solid color = Texture** and switches **X-Ray and the Retopology overlay back off** — neither is this tool's to turn on (why, below). Material Preview / Rendered viewports are untouched. **The plane sits 5 mm below z = 0**, so anything else lying flat at z = 0 (the Guide in its Flat SK pose, the flat retopo, a hand-made bake board) covers it from above. The retopo is the one you can't hide, because it's what you're cutting — use **Add Transparent Material** for that. Hide the rest **by hand**; this button does not touch other objects' visibility |
| **Add Transparent Material** | puts a transparent working material, `AC9_RetopoTransparent`, on the Retopo Mesh, so the Preview Plane 5 mm below reads through the faces you're cutting | Object Mode / Retopo | the file's existing `AC9_RetopoTransparent` (or the older `AC9_RetopoGhost`) is **reused** when there is one, keeping the alpha already set on it. **Refused when the retopo carries any other material** — which slot is which is your business, and rearranging it is not this tool's call. It also switches the **Retopology overlay off in every workspace**, since that overlay overrides the material's transparency (**Plane** only touches the workspace you're looking at, so this one reaches wider). While the material is on, this row becomes the **Alpha** slider and a remove (×) button |
| **Alpha** | how see-through the working material makes the retopo (0 = invisible, 1 = solid; default 0.35) | the material above is on | the value is kept on the `AC9_RetopoTransparent` material itself, so it is **saved in the .blend** and **nothing in Preferences is touched**. Solid and Material Preview read transparency from different properties (measured), so this slider writes both |
...
**Why the Retopology overlay is not used**: it replaces the edit-mesh face fill with the theme's `face_retopology` colour (measured alpha 0.502). While it is on, **the transparent material does nothing in Edit Mode** — the retopo cannot be made any thinner than the theme says, and that theme is a **Preference shared by every file**. On top of that, `show_retopology` is stored per 3D View **inside the .blend** (measured: it survives a save and reload), so files an earlier version of this add-on touched are still carrying it. **Plane** therefore switches it back off when it finds it, the same way it does with X-Ray.
...
| **Curvature Radius** | the scale of detail (mm, default 8) the drape Curvature reports. The Guide is smoothed with a kernel this wide and the map shows how far the surface sits above that smoothed copy along its normal: **white is convex (a ridge), black concave (a fold), mid grey flat**. Features much wider than the radius are smoothed away together with the reference and disappear, so set it near the width of the ridges you cut along. Smaller picks up the weave's own micro-relief as grain; larger melts neighbouring folds together. Cost rises with the square of the radius (measured at 172k verts: 0.98 s at 8 mm, 5.4 s at 20 mm, against 2.7 s for the 2K bake itself) |
