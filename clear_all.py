...
over the whole file:
  ...
  * on every mesh: the ``ac9_*`` / ``AC9_*`` attribute layers (attachments,
    ...
    ``AC9_Project_Failed`` vertex group, the ``AC9_Island_Colors`` slot
...
generic sweep possible for attributes and custom properties. Datablocks
(images, materials, texts) are matched by their exact names instead, because
``AC9`` is also a product name the user may use in their own naming.
"""
...
IMAGE_NAMES = (_bcore.RESIDUAL_IMAGE, _bcore.SAG_IMAGE, _bcore.DRAPE_IMAGE,
               _bcore._DRAPE_AO_TEMP, _bcore._DRAPE_CURVATURE_TEMP)
...
def run(scene, dry_run=True):
    ...
    rep = {
        ...
        "shape_keys": 0, "baked_3d": [], "vertex_groups": 0,
    ...
    # ---- datablocks by exact name
    for coll, wanted, key in ((bpy.data.images, IMAGE_NAMES, "images"),
                              (bpy.data.materials, MATERIAL_NAMES, "materials"),
...
def _drop_caches():
    ...
    for fn in (
        ...
        _proj_overlay.invalidate,
        _proj_overlay.invalidate_boundary,
        _seam_overlay.clear_topo_corner_batch,
        _seam_overlay.clear_density_pin_batch,
        _seam_overlay.clear_anchor_batch,
        _seam_overlay.clear_seam_status_batches,
    ):
        try:
            fn()
        except Exception:
