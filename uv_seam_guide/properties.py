...
class AC9SeamGuideProps(PropertyGroup):
    ...
    use_marked_seams: BoolProperty(
        ...
        description=(
            ...
            "hard edges) and unexpected guides appear"
        ),
        default=True,
    )
    ...
    max_distance_uv: FloatProperty(
        ...
        description=(
            "Maximum distance (flat-layout world units, ~metres) for a retopo "
    ...
    bond_distance: FloatProperty(
        ...
        description=(
            ...
            "(placed → green). Separate from Snap Distance, which is only the "
            "interactive Ghost Snap drag radius. Keep this small (a few mm): it "
            "is also the radius Force Bond uses to pull selected verts exactly "
            "onto their ghost, so a large value would yank unrelated verts onto "
            "the seam and distort faces"
        ),
    ...
    snap_distance: FloatProperty(
        name="Snap Distance",
        description=(
            "Maximum distance (guide-space units) for a moving vertex to snap to a ghost. "
            "Enable 'Show Snap Radius' to visualize this as circles around each ghost"
        ),
    ...
    show_seam_status: BoolProperty(
        ...
        description=(
            "Colour each sewn seam by whether the retopo pairs up on it: "
            "green = both sides authored and aligned, red = both sides but "
            "they disagree, orange = only one side done. Seams with no retopo "
            "yet are left uncoloured. Run 'Seam Status' to fill it in"
        ),
        default=True,
        update=prop_redraw,
    )
    ...
    gen_spacing_mm: FloatProperty(
        name="Spacing (mm)",
        description="Roughly how far apart the generated vertices should sit",
    ...
    fill_target_mm: FloatProperty(
        ...
        description=(
            "Interior quad size for the preview fill. 0 follows the boundary "
            "Spacing above, which is what makes the preview honest: the "
            "interior then shows the density the boundary is asking for"
        ),
    ...
    grid_target_mm: FloatProperty(
        ...
        description=(
            "Cell size for the structured grid. 0 follows the boundary "
            "Spacing above"
        ),
        default=0.0, min=0.0, soft_max=100.0,
    )
    ...
    retopo_sync_tol: FloatProperty(
        ...
        description=(
            ...
            "the middle of a panel and are left alone"
        ),
