...
def draw_boundary_object_tools(layout, context, props):
    """Whole-mesh operations (Object Mode): generate, sync, verify, preview.

    No hidden prerequisite here: Generate, Sync and Seam Status recompute
    the Guide's spans (and Generate its folds) on every run, so they do not
    ...
    uic.draw_check_apply(row, "ac9_cloth.sync_retopo_seams",
                         apply_text="Sync", apply_icon='ADD')
...
def draw_boundary_edit_tools(layout, context, props):
    """Selection-driven operations (Edit Mode): density, pins, snapping."""
    ...
    col = layout.column(align=True)
    row = uic.labeled_row(col, "Selected")
    op_check, op_apply = uic.draw_check_apply(
        row, "ac9_cloth.sync_selected_vertex",
        apply_text="Sync+Pin", apply_icon='PINNED')
    op_check.pin = True
    op_apply.pin = True
...
class AC9_PT_Boundary(bpy.types.Panel):
    """The retopo's 2D boundary. Shows the tools of the Retopo's current mode
    first and folds the other mode's tools into one line.
    """
    bl_space_type  = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_category    = "AC9 Cloth Retopo"
    ...
    def draw(self, context):
        ...
        if uic.retopo_mode(top) == 'EDIT':
            draw_boundary_edit_tools(layout, context, props)
            other = uic.draw_fold(layout, top, "ui_boundary_show_other",
                                  "Object Mode tools")
            if other is not None:
                draw_boundary_object_tools(other, context, props)
        else:
            draw_boundary_object_tools(layout, context, props)
            other = uic.draw_fold(layout, top, "ui_boundary_show_other",
                                  "Edit Mode tools")
            if other is not None:
                draw_boundary_edit_tools(other, context, props)
...
class AC9_PT_BoundarySettings(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.separator()
        col = layout.column(align=True)
        col.label(text="Sync", icon='MOD_LENGTH')
...
def draw_advanced(layout, context):
    ...
    col = layout.column(align=True)
    col.label(text="Ghosts", icon='GHOST_ENABLED')
    col.prop(props, "live_ghost_update", toggle=True, icon='TEMP')
