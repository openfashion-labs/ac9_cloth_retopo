...
def divide_interior_lines(bm, spacing_mm, comp_edges=None):
    ...
    `spacing_mm` is in the FLAT layout's millimetres, not real fabric ones --
    this measures the retopo, which lives in the flat layout. The caller
    converts the user's real-mm setting with
    clo_projector.guide.flat_scale; passing the raw setting is the bug
    AUDIT §8-D-1 records (a 20 mm setting produced a 21.95 mm boundary against
    a 20.00 mm interior on a shirt whose flat layout is 1.1245x real scale).
    """
...
def run_grid_regions(retopo, target_mm, smooth_passes=6, concave_deg=20.0,
    ...
    `progress`, if given, is called with a 0..1 fraction at each of the steps
    above and once per region in the last one — this takes seconds on a whole
    object, and the press showed nothing while it did. Pass a
    ...
    for i_reg, reg in enumerate(regions):
        tick(0.55 + 0.45 * (i_reg / max(1, n_regions)))
        sides = reg["sides"]
        ring = reg["ring"]
...
def connect_loose_ends(bm, mat, sources=None, snap_mm=2.0, within=None,
                       min_chain_mm=MIN_CHAIN_MM, progress=None):
    ...
    while pending:
        # Each round rays every end still waiting against every edge, so the
        # work per round grows with both — the reason this reaches a minute on
        # a whole object (measured: 65.6 s, 4405 verts).
        if progress is not None and n_pending:
            progress((n_pending - len(pending)) / n_pending)
        best = None
        for ray in pending:
            r = first_hit(*ray)
...
def run_connect_loose_ends(retopo, selected_only=False, extend_selected=False,
    ...
    `progress`, if given, is called with a 0..1 fraction at each step and once
    per loose end resolved in the long one. Pass a ui_common.ProgressThrottle,
    never a raw progress_update.
    """
    def tick(fraction):
        if progress is not None:
            progress(fraction)
    ...
    rep = connect_loose_ends(bm, mat, sources=sources, snap_mm=snap_mm,
                             within=within,
                             progress=lambda f: tick(0.25 + 0.60 * f))
    tick(0.85)
    clear_scaffold(bm, within=within)
    bm.verts.ensure_lookup_table()
